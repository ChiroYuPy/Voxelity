...
void ChunkMeshingRequestManager::enqueueDirtyChunks(ChunkStorage& worldChunkData) const {
    for (const auto& chunkPtr : worldChunkData.chunks | std::views::values) {
        ...
        std::array<VoxelStorage*, 6> dataNbrs{};
        for (int i = 0; i < 6; ++i)
...
void ChunkMeshingRequestManager::processReadyMeshes(ChunkStorage& worldChunkData) const {
    glm::ivec3 pos;
